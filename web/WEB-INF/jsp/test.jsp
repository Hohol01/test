            ...
            <c:forEach var="tests" items="${tests}">
                <tr>
                    ...
                    <td <% if (request.getSession().getAttribute("role").equals("teacher"))
                        out.print("hidden");%> ><a href="passingtest?idtest=${tests.id}"><fmt:message
                            key="test_jsp.pass_test"/> </a></td>

                    <td <% if (request.getSession().getAttribute("role").equals("student"))
                        out.print("hidden");%> ><a href="edit?idtest=${tests.id}"><fmt:message
                            key="test_jsp.edit_test"/> </a></td>
                    <td <% if (request.getSession().getAttribute("role").equals("student"))
